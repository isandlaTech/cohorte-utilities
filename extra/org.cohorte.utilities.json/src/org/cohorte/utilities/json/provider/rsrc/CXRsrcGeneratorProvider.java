...
public class CXRsrcGeneratorProvider extends CXRsrcProvider {
	...
	private JSONObject applyGenerator(final JSONObject aGenerator, final List<JSONObject> aListOfFatherJson)
			throws JSONException {
		final JSONObject wApplied = new JSONObject();
		for (final String aProp : aGenerator.keySet()) {
			final Object wValue = aGenerator.opt(aProp);
	...
	private void applyGeneratorProperties(final JSONObject aApplied, final String aProp, final JSONObject aGenerator,
		...
		if (aValue instanceof String) {
			...
			while (wMatcher.find()) {
				final String wMatch = wMatcher.group();
				...
				if (aListOfFatherJson == null) {
					...
					final Pair<String, JSONObject> wTuple = getFather(wMatch, aListOfFatherJson);
			...
			final JSONArray wArrayValue = (JSONArray) aValue;
			final JSONArray wReplaceArrayValue = new JSONArray();
			for (int i = 0; i < wArrayValue.length(); i++) {
				final Object wElem = wArrayValue.opt(i);
				if (wElem instanceof JSONObject) {
					...
					final Matcher wMatcher = pPatternJsonPath.matcher((String) wElem);
	...
	private String applyJsonPath(final String aJson, final String aJsonPath) {
		try {
			...
			// array
			final Matcher wMatcher = pPatternConditionJsonPath.matcher(aJsonPath);
			while (wMatcher.find()) {
				String wCond = wMatcher.group();
				final int wIndexCond = wJsonPath.indexOf(wCond);
				final Object wObj = JsonPath.read(aJson, aJsonPath.substring(0, wIndexCond));
				if (wObj.toString().startsWith("[")) {
					final JSONArray wArr = new JSONArray(wObj.toString());
					wCond = wCond.substring(1, wCond.length() - 1);
					final String[] wCondSpl = wCond.split("=");
					final int wSubJsonIndex = findJSONObject(wCondSpl[0], wCondSpl[1], wArr);
					wJsonPath = wJsonPath.replace(wCond, Integer.toString(wSubJsonIndex));
				}
			}
			final Object wObj = JsonPath.read(aJson, wJsonPath);
	...
	public CXRsrcProvider clone() {
		final CXRsrcGeneratorProvider wRsrc = new CXRsrcGeneratorProvider(pActivityLogger);
		return wRsrc;
	}
	 ...
	 *
	 * @param aKey : a json property name of the json
	 * @param aKey : a value associate to the key
	 * @param aArr : a json array
	 * @return
	 */
	private int findJSONObject(final String aKey, final String aValue, final JSONArray aArr) {
		...
		for (int i = 0; aArr != null && i < aArr.length() && wFound == 0; i++) {
			final JSONObject wItem = aArr.optJSONObject(i);
			if (wItem != null) {
				final Object wKey = wItem.opt(aKey);
	 ...
	 *
	 * @param aJsonPathWithFather : jsonpath with dscription of father using
	 *                            relative path
	 *
	 * @param aListOfFather
	...
	private Pair<String, JSONObject> getFather(final String aJsonPathWithFather, final List<JSONObject> aListOfFather) {
		...
		final Pair<String, String> wTuple = getPathFather(aJsonPathWithFather);
	...
	public CXRsrcText rsrcReadTxt(final String aGeneratorContent) throws Exception {
		final JSONObject wGeneratorContent = new JSONObject(aGeneratorContent);
		final JSONObject wAppliedContent = applyGenerator(wGeneratorContent, null);
	...
	public CXRsrcText rsrcReadTxt(final String aGeneratorContent, final List<JSONObject> aListFather) throws Exception {
		final JSONObject wGeneratorContent = new JSONObject(aGeneratorContent);
		final JSONObject wAppliedContent = applyGenerator(wGeneratorContent, aListFather);
		if (wAppliedContent.has("cond")) {
			wAppliedContent.remove("cond");
		}
		return new CXRsrcText(new CXRsrcUriPath(""),
