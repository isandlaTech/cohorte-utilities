...
public class CHandlerMemoryCacheSchema implements IHandlerInitMemoryCache {
	...
	@Override
	public void initCache(final Object aContent,
			...
			if (aContent instanceof JSONObject) {
				...
				if (wDef != null) {
					for (String wKey : wDef.keySet()) {
						String wStrToCache = wDef.getJSONObject(wKey).toString(
								2);
						String wFinalKey = PREFIX_DEF_KEY + wKey;
						CXRsrcText wRsrcToCache = new CXRsrcText(
								new CXRsrcUriPath(wKey),
								CXRsrcTextReadInfo
