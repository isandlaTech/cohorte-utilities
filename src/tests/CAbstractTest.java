...
public abstract class CAbstractTest {
	 ...
	 * @throws Exception
	 */
	protected abstract void doCommandClose(final String aCmdeLine) throws Exception;
	...
	protected boolean isCommandClose(final String aCmdeLine){
		return isCommandX(aCmdeLine,CMD_CLOSE) || isCommandX(aCmdeLine,CMD_QUIT)  ;
	}
	...
	protected boolean isCommandX(final String aCmdeLine,final String aCommandId){
		...
		return wCmdeArgs[0].toLowerCase().equals(wCommand.getVerb()) || wCmdeArgs[0].toLowerCase().equals(wCommand.getAlias());
	}	
...
class CCommand implements Comparable<CCommand>{
	 ...
	 */
	@Override
	public int compareTo(CCommand aCommand) {
